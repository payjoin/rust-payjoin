...
impl App {
    ...
    async fn handle_payjoin_post(
        ...
        let body = read_limited_body(body.into_data_stream(), MAX_CONTENT_LENGTH)
            .await
...
pub async fn read_limited_body<S, E>(mut stream: S, max_len: usize) -> Result<Vec<u8>>
where
    ...
    E: std::error::Error + Send + Sync + 'static,
{
    ...
    while let Some(chunk) = stream.next().await {
        ...
        if body.len() + chunk.len() > max_len {
            return Err(anyhow!("Body exceeds maximum allowed size of {max_len} bytes"));
        }
