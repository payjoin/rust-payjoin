    ...
    async fn resume_payjoins(&self) -> Result<()> {
        ...
        for session_id in recv_session_ids {
            ...
            match replay_receiver_event_log(&recv_persister) {
                ...
                Err(e) => {
                    tracing::debug!("An error {:?} occurred while replaying receiver session", e);
                    // Close the session when replay fails
                    if let Err(close_err) = SessionPersister::close(&recv_persister) {
                        tracing::warn!(
                        ...
                        println!("Closed failed receiver session: {}", session_id);
                    }
                }
            }
        }
        ...
        for session_id in send_session_ids {
            ...
            match replay_sender_event_log(&sender_persiter) {
                ...
                Err(e) => {
                    tracing::debug!("An error {:?} occurred while replaying Sender session", e);
                    // Close the session when replay fails
                    if let Err(close_err) = SessionPersister::close(&sender_persiter) {
                        tracing::warn!(
                            "Failed to close sender session {}: {:?}",
                            session_id,
                            close_err
                        );
                    } else {
                        println!("Closed failed sender session: {}", session_id);
                    }
                }
            }
        }
