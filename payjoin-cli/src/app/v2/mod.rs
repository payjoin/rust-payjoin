...
/// types so the relay-send helper can hand expiry back to the caller (which
/// owns the typestate needed to react) instead of flattening it into
/// `anyhow::Error`.
trait RequestExpiry {
...
/// Outcome of building and posting a request via selected OHTTP relays. HTTP
/// failures are retried against other selected relays inside the helper; only
/// session expiry and fatal build errors escape to the caller.
enum RelayPost<T> {
...
impl App {
    ...
    async fn post_original_proposal(
        ...
        let (response, ctx) = match self
            .send_ohttp_request_with_relay_selector(relay_selector, RequestKind::Post, |relay| {
                sender.create_v2_post_request(relay.as_str())
            })
            .await?
        {
    ...
    async fn get_proposed_payjoin_psbt(
        ...
        let (response, ctx) = match self
            .send_ohttp_request_with_relay_selector(relay_selector, RequestKind::Poll, |relay| {
                sender.create_poll_request(relay.as_str())
            })
            .await?
        {
            RelayPost::Posted(resp, ctx) => (resp, ctx),
    ...
    async fn read_from_directory(
        ...
        let (ohttp_response, context) = match self
            .send_ohttp_request_with_relay_selector(relay_selector, RequestKind::Poll, |relay| {
                session.create_poll_request(relay.as_str())
            })
            .await?
        {
            RelayPost::Posted(resp, ctx) => (resp, ctx),
    ...
    async fn send_payjoin_proposal(
        ...
        let (res, ohttp_ctx) = match self
            .send_ohttp_request_with_relay_selector(relay_selector, RequestKind::Post, |relay| {
                proposal.create_post_request(relay.as_str())
            })
            .await?
        {
            RelayPost::Posted(resp, ctx) => (resp, ctx),
    ...
    async fn handle_error(
        ...
        let (err_response, err_ctx) = match self
            .send_ohttp_request_with_relay_selector(relay_selector, RequestKind::Post, |relay| {
                session.create_error_request(relay.as_str())
            })
            .await?
        {
            RelayPost::Posted(resp, ctx) => (resp, ctx),
    ...
    async fn send_ohttp_request_with_relay_selector<F, T, E>(
        &self,
        relay_selector: &RelaySelector,
    ...
    where
        F: FnMut(&payjoin::Url) -> std::result::Result<(payjoin::Request, T), E>,
        E: RequestExpiry + Into<anyhow::Error>,
    {
        ...
        while let Some(relay) = relays.next() {
            ...
            let (req, ctx) = match build(&relay.url) {
                Ok(r) => r,
