...
impl App {
    ...
    /// Drive the sender state machine to completion. Each step function
    /// performs a single state transition and returns the next session
    /// state, so retries and polling loop through here rather than
    /// recursing.
    ...
    /// Drive the receiver state machine to completion. Each step function
    /// performs a single state transition and returns the next session
    /// state, so retries and polling loop through here rather than
    /// recursing.
        ...
        loop {
            session = match session {
                ...
                ReceiveSession::Monitor(proposal) =>
                    return self.monitor_payjoin_proposal(proposal, persister).await,
            ...
            Err(e) => {
                ...
                match e.error_state() {
                    Some(session) => {
                        ...
                        Ok(ReceiveSession::PendingFallback(session))
                    }
