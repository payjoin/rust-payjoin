        ...
        async fn expire_sender_async(services: &TestServices, temp_dir: &TempDir) -> Result<()> {
            ...
            let broadcast_line = tokio::time::timeout(
                ...
                wait_for_stdout_match(&mut sender_stdout, |l| {
                    l.contains("Broadcasted fallback transaction")
                }),
            )
            .await?;
            ...
            let broadcast_output =
                broadcast_line.expect("sender should broadcast fallback tx on expiry");
            let fallback_txid = broadcast_output.split_whitespace().nth(7).unwrap_or("");

            let fallback_txid = Txid::from_str(fallback_txid).expect("valid txid");

            assert!(
                sender.get_raw_transaction(fallback_txid).is_ok(),
                "fallback tx should be in the mempool after expiry auto-cancel"
            );
    ...
    async fn receiver_expire_v2() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        async fn expire_receiver_async(services: &TestServices, temp_dir: &TempDir) -> Result<()> {
            ...
            let expired_line = tokio::time::timeout(
                timeout,
                wait_for_stdout_match(&mut receiver_stdout, |l| {
                    l.contains("Receiver session expired")
                }),
            )
            .await?;
