...
#[derive(Clone)]
pub struct Db(Arc<Mutex<Mailboxes>>);

impl Db {
    pub fn new(timeout: Duration) -> Self { Self(Arc::new(Mutex::new(Mailboxes::new(timeout)))) }
}
...
impl Mailboxes {
    fn get(&mut self, id: &ShortId) -> Option<Arc<Vec<u8>>> {
        ...
        self.populated.entry(*id).and_modify(|entry| {
            ...
            if entry.metadata.read.is_some() {
                entry.mark_read();
...
impl crate::db::Db for Db {
    ...
    async fn wait_for_v2_payload(
        &self,
        id: &ShortId,
    ) -> Result<Arc<Vec<u8>>, crate::db::Error<Self::OperationalError>> {
