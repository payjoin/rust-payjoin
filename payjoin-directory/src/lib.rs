...
impl Service {
    ...
    where
        I: tokio::io::AsyncRead + tokio::io::AsyncWrite + Send + Unpin + 'static, // FIXME why static?
    {
        if let Err(err) =
...
pub async fn serve_metrics_tcp(service: Service, listener: TcpListener) -> Result<(), BoxError> {
    while let Ok((stream, _)) = listener.accept().await {
