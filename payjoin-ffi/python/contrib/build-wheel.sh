...
#
# Both wheels build on a Linux host, like the C# native assets: a dylib
# linked inside the nix dev shell on a mac records nix store install names
# that exist on no user machine, while cargo-zigbuild links against zig's
# bundled Apple SDK stubs and records the system ones.
