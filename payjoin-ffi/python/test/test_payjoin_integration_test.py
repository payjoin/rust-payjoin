...
class TestPayjoin(unittest.IsolatedAsyncioTestCase):
    ...
    def create_receiver_context(self, receiver_address: bitcoinffi.Address, directory: Url, ohttp_keys: OhttpKeys, recv_persister: InMemoryReceiverSessionEventLog) -> Initialized:
        receiver = UninitializedReceiver().create_session(address=receiver_address, directory=directory.as_string(), ohttp_keys=ohttp_keys, expire_after=None, max_fee_rate_sat_per_vb=None).save(recv_persister)
        return receiver
    ...
    async def process_wants_fee_range(self, proposal: WantsFeeRange, recv_persister: InMemoryReceiverSessionEventLog):
        provisional_proposal = proposal.apply_fee_range(None, None).save(recv_persister)
