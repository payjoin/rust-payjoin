...
mod v2 {
    ...
    async fn v2_to_v2_full_cycle() {
        ...
        async fn do_v2_send_receive(services: &TestServices) -> Result<(), BoxError> {
            ...
            let (request, client_response) = session.create_poll_request(ohttp_relay.to_string())?;
            let response = agent
            ...
            let (request, context) = req_ctx.create_v2_post_request(ohttp_relay.to_owned().into())?;
            let response = agent
                .post(request.url.as_string())
                .header("Content-Type", request.content_type)
