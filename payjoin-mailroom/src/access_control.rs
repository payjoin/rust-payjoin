...
mod tests {
    ...
    fn test_geo_reader() -> maxminddb::Reader<Vec<u8>> {
        // Keep a tiny MMDB fixture in-repo so GeoIP tests are offline and deterministic while
        // still exercising real MaxMind DB parsing/lookup behavior.
    ...
    fn check_ip_allows_when_no_geo_reader() {
        let ac = AccessControl { geo_reader: None, blocked_regions: HashSet::new() };
    ...
    fn check_ip_allows_when_no_blocked_regions() {
        ...
        let ac = AccessControl { geo_reader: Some(reader), blocked_regions: HashSet::new() };
    ...
    fn check_ip_blocks_blocked_region() {
        ...
        let ac = AccessControl { geo_reader: Some(reader), blocked_regions };
        assert!(!ac.check_ip("2.125.160.216".parse().unwrap()));
    }
