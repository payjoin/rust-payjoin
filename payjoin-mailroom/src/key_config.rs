...
/// Set the modification time of the key file for `key_id` in `dir`.
///
/// The mtime is how restarts re-derive `activated_at`/`valid_until` for the
/// active key, so callers stamp it at the moment a key becomes the served
pub async fn set_key_mtime(
    dir: &Path,
    key_id: u8,
    mtime: std::time::SystemTime,
) -> std::io::Result<()> {
    let path = key_path_for_id(dir, key_id);
    let times = fs::FileTimes::new().set_modified(mtime);
    let std_file = tokio::fs::File::open(&path).await?.into_std().await;
    tokio::task::spawn_blocking(move || std_file.set_times(times))
        .await
        .map_err(std::io::Error::other)?
}

/// Remove any existing key file for `key_id` in `dir`, generate a fresh
/// `ServerKeyConfig` for that id, persist it, and return the config.
pub async fn regenerate_key_config(dir: &Path, key_id: u8) -> Result<ServerKeyConfig> {
    let _ = tokio::fs::remove_file(key_path_for_id(dir, key_id)).await;
    let config = gen_ohttp_server_config_with_id(key_id)?;
    persist_key_config(&config, dir).await?;
    Ok(config)
}
