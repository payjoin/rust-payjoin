...
impl TryFrom<&str> for Url {
    type Error = Error;
    fn try_from(s: &str) -> Result<Self> { Ok(Url(url::Url::parse(s)?)) }
}

impl TryFrom<String> for Url {
    type Error = Error;
    fn try_from(s: String) -> Result<Self> { Url::try_from(s.as_str()) }
}
...
impl<'de> Deserialize<'de> for Url {
        ...
        D: Deserializer<'de>,
    {
        ...
        Url::try_from(s).map_err(de::Error::custom)
    }
}
...
mod tests {
    ...
    fn into_url_file_scheme() {
        let err = Url::try_from("file:///etc/hosts").unwrap().into_url().unwrap_err();
        assert_eq!(err.to_string(), "URL scheme is not allowed");
    }

    #[test]
    fn into_url_blob_scheme() {
        let err = Url::try_from("blob:https://example.com").unwrap().into_url().unwrap_err();
        assert_eq!(err.to_string(), "URL scheme is not allowed");
    }
