    ...
    pub(crate) fn deconstruct(
        ...
        match self.0 {
            ...
            Ok(AcceptOptionalTransition::NoResults(current_state)) =>
                (PersistActions::None, Ok(OptionalTransitionOutcome::Stasis(current_state))),
            ...
            Err(Rejection::Transient(RejectTransient(error))) =>
                (PersistActions::None, Err(ApiError::Transient(error))),
...
impl<Event, NextState, Err> MaybeTransientTransition<Event, NextState, Err> {
    ...
    pub(crate) fn deconstruct(self) -> (PersistActions<Event>, Result<NextState, ApiError<Err>>) {
        match self.0 {
            Ok(AcceptNextState(event, next_state)) => (PersistActions::Save(event), Ok(next_state)),
            Err(RejectTransient(error)) => (PersistActions::None, Err(ApiError::Transient(error))),
        }
    }
...
#[derive(Debug)]
pub(crate) enum ApiError<E, ErrorState = ()> {
...
impl<E, StorageErr, ErrorState> From<ApiError<E, ErrorState>>
    for InternalPersistedError<E, StorageErr, ErrorState>
where
    E: std::error::Error,
    StorageErr: std::error::Error,
    ErrorState: fmt::Debug,
{
    fn from(api: ApiError<E, ErrorState>) -> Self { InternalPersistedError::Api(api) }
}
