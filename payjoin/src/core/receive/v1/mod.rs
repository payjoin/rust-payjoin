...
impl UncheckedOriginalPayload {
    ...
    pub fn check_broadcast_suitability(
        ...
        self.apply_broadcast_suitability(
            min_fee_rate,
            can_broadcast(&tx).map_err(Error::Implementation)?,
        )
    }
...
impl ProvisionalProposal {
    ...
    pub fn finalize_proposal(
        ...
        let signed_psbt = wallet_process_psbt(&psbt).map_err(Error::Implementation)?;
    ...
    pub fn finalize_signed_proposal(self, signed_psbt: &Psbt) -> Result<PayjoinProposal, Error> {
        let finalized_psbt = self
            .psbt_context
            .finalize_signed_proposal(signed_psbt.clone())
            .map_err(Error::Implementation)?;
