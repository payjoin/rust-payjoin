...
impl UncheckedOriginalPayload {
    ...
    {
        let can_broadcast_sync = self
            .original
            .to_sync_can_broadcast(can_broadcast)
            .await
...
impl MaybeInputsOwned {
    ...
    where
        F: Future<Output = Result<bool, ImplementationError>>,
    {
        let mut is_owned_sync = self.original.to_sync_is_owned(is_owned).await?;
...
impl MaybeInputsSeen {
        ...
        F: Future<Output = Result<bool, ImplementationError>>,
    {
        let mut is_known_sync =
            self.original.to_sync_is_known(is_known).await.map_err(Error::Implementation)?;
...
impl ProvisionalProposal {
    ...
    {
        let wallet_process_psbt_sync = self
            .psbt_context
            .to_sync_wallet_process_psbt(wallet_process_psbt)
            .await
            .map_err(|e| Error::Implementation(ImplementationError::new(e)))?;
        self.finalize_proposal(wallet_process_psbt_sync)
    }
