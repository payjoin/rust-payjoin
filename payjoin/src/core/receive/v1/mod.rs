...
use crate::receive::{InternalPayloadError, Proposal, PsbtContext};
...
impl UncheckedProposal {
    ...
    pub fn check_broadcast_suitability(
        ...
        match self.proposal.clone().check_broadcast_suitability(min_fee_rate, can_broadcast) {
            Ok(()) => Ok(MaybeInputsOwned { proposal: self.proposal }),
            Err(e) => Err(e),
        }
    }
    ...
    pub fn assume_interactive_receiver(self) -> MaybeInputsOwned {
        MaybeInputsOwned { proposal: self.proposal }
    }
}
...
impl MaybeInputsOwned {
    ...
    pub fn extract_tx_to_schedule_broadcast(&self) -> bitcoin::Transaction {
        self.proposal.psbt.clone().extract_tx_unchecked_fee_rate()
    }
    ...
    pub fn check_inputs_not_owned(
        ...
        self.proposal.clone().check_inputs_not_owned(is_owned)?;
        Ok(MaybeInputsSeen { proposal: self.proposal })
    }
}
...
impl MaybeInputsSeen {
    ...
    pub fn check_no_inputs_seen_before(
        ...
        self.proposal.clone().check_no_inputs_seen_before(is_known)?;
        Ok(OutputsUnknown { proposal: self.proposal })
    }
}
...
impl OutputsUnknown {
    ...
    pub fn identify_receiver_outputs(
        ...
        let owned_vouts = self.proposal.clone().identify_receiver_outputs(is_receiver_output)?;
        ...
        Ok(WantsOutputs::from_proposal(self.proposal, owned_vouts))
    }
}
...
impl WantsOutputs {
    ...
    pub(crate) fn from_proposal(proposal: Proposal, owned_vouts: Vec<usize>) -> Self {
        Self {
...
pub(crate) mod test {
    ...
    pub(crate) fn unchecked_proposal_from_test_vector() -> UncheckedProposal {
        ...
        let params = Params::from_query_pairs(pairs, &[Version::One])
            .expect("Could not parse params from query pairs");
        UncheckedProposal { proposal: Proposal { psbt: PARSED_ORIGINAL_PSBT.clone(), params } }
    }
    ...
    pub(crate) fn maybe_inputs_owned_from_test_vector() -> MaybeInputsOwned {
        ...
        let params = Params::from_query_pairs(pairs, &[Version::One])
            .expect("Could not parse params from query pairs");
        MaybeInputsOwned { proposal: Proposal { psbt: PARSED_ORIGINAL_PSBT.clone(), params } }
    }
    ...
    fn is_output_substitution_disabled() {
        ...
        proposal.proposal.params.output_substitution = OutputSubstitution::Disabled;
    ...
    fn unchecked_proposal_min_fee() {
        ...
        let min_fee_rate =
            proposal.proposal.psbt_fee_rate().expect("Feerate calculation should not fail");
        let _ = proposal
            .clone()
    ...
    fn unchecked_proposal_unlocks_after_checks() {
        let proposal = unchecked_proposal_from_test_vector();
        assert_eq!(proposal.proposal.psbt_fee_rate().unwrap().to_sat_per_vb_floor(), 2);
    ...
    fn sender_specifies_excessive_fee_rate() {
        ...
        proposal.proposal.params.min_fee_rate = FeeRate::from_sat_per_vb_unchecked(1000);
    ...
    fn test_prepare_psbt_excludes_keypaths() {
        let proposal = unchecked_proposal_from_test_vector();
        let mut processed_psbt = proposal.proposal.psbt.clone();
