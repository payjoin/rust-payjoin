...
/// [`ChainView`] currently shows about the contested outpoints. It is recomputed on every
/// [`Receiver<Monitor>::classify`] call and is never persisted. The caller decides — based on the
/// `Detected` confirmation count — when its confidence bar is met and the session should
/// [`Receiver<Monitor>::conclude`].
///
/// Confirmation depth is a confidence dial, not a state transition: the payjoin proposal and the
/// fallback double-spend the sender's inputs, so a deep reorg or the conflicting transaction can
/// always flip a `Detected` verdict until it is buried deep enough for the caller's purposes.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum SettlementStatus {
    ...
    Detected {
        ...
        /// The most conservative (minimum) confirmation count over the spends that support
        /// `outcome`. `0` means mempool-only.
        confirmations: u32,
    },
}
...
///
/// This enum is `#[non_exhaustive]`: settlement attribution may grow finer terminal labels in a
/// minor release, so downstream matches must carry a wildcard arm.
///
/// Outcome precedence is **Cooperative > Fallback > Other** (see [`Receiver<Monitor>::classify`]).
#[derive(Debug, Clone, PartialEq, Eq)]
#[non_exhaustive]
pub enum SettlementOutcome {
    /// The Payjoin settled cooperatively: the spending transaction either structurally contains the
    /// receiver's contributed input (the payjoin fingerprint) or, for an output-substitution
    /// proposal, reproduces the proposal's output set (a cut-through). Either signal proves the
    /// receiver's contribution was accepted and settled — a successful, privacy-improving outcome.
    ///
    /// Detection is structural, not txid-based, so it matches even when the spending transaction's
    /// txid differs from the predicted Payjoin txid (the non-SegWit case earlier versions could not
    /// monitor). The caller already holds the full spending transaction it passed into
    /// [`classify`](Receiver::classify) via the [`ChainView`], so rebroadcasting or retaining the
    /// cooperative transaction as evidence is the caller's to do — mirroring how the fallback path
    /// hands the caller a complete transaction via
    /// [`extract_tx_to_schedule_broadcast`](Receiver::extract_tx_to_schedule_broadcast).
    Cooperative,
...
///
/// The wallet (not the library) is the oracle for which outpoint is spent by what, at what depth. A
/// spend always carries its full transaction ([`Spend::tx`]); the spending txid is derived from it
/// ([`bitcoin::Transaction::compute_txid`]) rather than tracked separately, so an outpoint can
/// never be reported as spent without the transaction body that structural classification needs.
#[derive(Debug, Clone, PartialEq, Eq)]
...
///
/// The transaction is mandatory. Structural cooperative classification (the input fingerprint or
/// the output-substitution match) needs the body, and the spending txid is derived from it via
/// [`bitcoin::Transaction::compute_txid`] rather than carried alongside, so the two can never
/// disagree.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Spend {
...
impl Receiver<Monitor> {
    ...
    /// The sender's original inputs — the outpoints both the Payjoin proposal and the fallback
    /// contend to spend. Only one transaction can ever spend them, which is the double-spend race
    /// at the heart of settlement attribution.
    ...
    /// only [`Receiver<Monitor>::conclude`] writes a terminal event.
    ///
    ...
    /// 2. A contested outpoint is spent by a transaction that proves the receiver's contribution
    ///    settled → [`SettlementOutcome::Cooperative`]. This holds for *either* cooperative signal:
    ///    the spending transaction contains the receiver's contributed input
    ///    ([`Receiver<Monitor>::receiver_input`], the input fingerprint), **or** — for an
    ///    output-substitution proposal — the spending transaction's output set matches the
    ///    proposal's outputs (order-independently), gated on the proposal actually substituting the
    ///    original outputs. Both are structural, so this salvages the non-SegWit case where the
    ///    on-chain txid differs from [`Receiver<Monitor>::payjoin_txid`]. The input-fingerprint step
    ///    is skipped when the proposal contributed no receiver input (an output-substitution-only
    ///    Payjoin).
    ...
    ///
    /// Tie-break: if contested outpoints are split across multiple transactions, the precedence
    /// above decides. A Cooperative wins if *any* spend matches either cooperative signal; else a
    /// Fallback wins if *any* spend is by the fallback txid; else the first unrecognized spend
    /// determines `Other`. The reported `confirmations` is the most conservative (minimum) over the
    /// spends that support the chosen outcome.
    pub fn classify(&self, view: &ChainView) -> SettlementStatus {
        let contested = self.contested_outpoints();

        // Only consider spends of contested outpoints that the caller reports as actually spent.
        ...
        // 2. Cooperative: any spend whose transaction proves the receiver's contribution settled,
        //    via either signal — the input fingerprint (the receiver's contributed input, when the
        //    proposal has one) or an order-independent match of the spending transaction's output
        //    set against the proposal's (a cut-through, gated on the proposal actually substituting
        //    the original outputs; otherwise the proposal is the fallback). Both signals are
        //    structural, so a txid change from a non-SegWit signature does not hide them.
        ...
        if !cooperative_spends.is_empty() {
            return SettlementStatus::Detected {
                outcome: SettlementOutcome::Cooperative,
    ...
    /// confidence bar — e.g. a confirmation depth observed via [`Receiver<Monitor>::classify`] — is
    /// met. This persists a single `Closed(SessionOutcome)` event and closes the session.
    ///
    /// Concluding is the caller's decision, not the library's: detection is necessary but not
    /// sufficient, and a 0-confirmation conclusion can still be reorged or out-raced by the
    /// conflicting (double-spending) transaction. The outcome maps to the persisted
    ...
    pub fn conclude(self, outcome: SettlementOutcome) -> TerminalTransition<SessionEvent, ()> {
        let session_outcome = match outcome {
            SettlementOutcome::Cooperative => SessionOutcome::Success,
    ...
    pub fn check_for_transaction(
        ...
        // This legacy path is the degenerate case of `classify` over a txid-only chain probe: it
        // can only look up the two transactions whose IDs the receiver predicts, so it cannot
        // surface `Other`. The first such transaction it finds concludes the session.
        ...
        match find_transaction(payjoin_txid) {
            Ok(Some(tx)) => {
                ...
                {
                    // Payjoin transaction was detected. Complete the session.
                    return MaybeFatalOrSuccessTransition::success(SessionEvent::Closed(
...
pub mod test {
    ...
    fn test_monitor_typestate() -> Result<(), BoxError> {
        ...
        assert_eq!(
            ...
            Some(&SessionEvent::Closed(SessionOutcome::Success))
        );
    ...
    /// The proposal transaction a [`cutthrough_monitor`] is built from (its substituted outputs and
    /// the sender's inputs). A spend whose output set matches this is a cut-through.
    ...
    fn test_classify_no_receiver_input_is_total() {
        // A proposal that contributes no receiver input must not panic in classify. Previously
        // `receiver_input` was unwrapped before the Pending guard, panicking on every call for an
        // output-substitution-only session even when nothing was spent.
    ...
    fn test_classify_cooperative_output_order_independent() {
        ...
        assert!(matches!(
            monitor.classify(&view),
            SettlementStatus::Detected { outcome: SettlementOutcome::Cooperative, .. }
        ));
    }
    ...
    fn test_classify_confirmations_are_conservative() {
        ...
        // classify reports the most conservative (minimum) confirmation count.
        let monitor = segwit_monitor();
        let payjoin_tx = PARSED_PAYJOIN_PROPOSAL.clone().unsigned_tx;
