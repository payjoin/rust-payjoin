...
use crate::receive::{parse_payload, InputPair, Proposal, PsbtContext};
...
impl Receiver<Initialized> {
    ...
    fn extract_proposal_from_v1(&mut self, response: &str) -> Result<Proposal, ReplyableError> {
        self.unchecked_from_payload(response)
    }

    fn extract_proposal_from_v2(&mut self, response: Vec<u8>) -> Result<Proposal, Error> {
    ...
    fn unchecked_from_payload(&mut self, payload: &str) -> Result<Proposal, ReplyableError> {
        ...
        let inner = Proposal { psbt, params };
        Ok(inner)
    }
...
impl Receiver<UncheckedProposal> {
    ...
    pub fn check_broadcast_suitability(
        ...
        match self.state.proposal.clone().check_broadcast_suitability(min_fee_rate, can_broadcast) {
            Ok(v1) => v1,
            Err(e) => match e {
...
impl Receiver<MaybeInputsOwned> {
    ...
    pub fn check_inputs_not_owned(
        ...
        match self.state.proposal.clone().check_inputs_not_owned(is_owned) {
            Ok(inner) => inner,
            Err(e) => match e {
...
impl Receiver<MaybeInputsSeen> {
    ...
    pub fn check_no_inputs_seen_before(
        ...
        match self.state.proposal.clone().check_no_inputs_seen_before(is_known) {
            Ok(inner) => inner,
            Err(e) => match e {
                ReplyableError::Implementation(_) => {
                    return MaybeFatalTransition::transient(e);
...
impl Receiver<OutputsUnknown> {
    ...
    pub fn identify_receiver_outputs(
        ...
        let owned_vouts =
            match self.state.proposal.clone().identify_receiver_outputs(is_receiver_output) {
                Ok(inner) => inner,
                Err(e) => match e {
                    ReplyableError::Implementation(_) => {
                        return MaybeFatalTransition::transient(e);
        ...
        let wants_outputs = v1::WantsOutputs::from_proposal(self.state.proposal, owned_vouts);
        MaybeFatalTransition::success(
...
pub mod test {
    ...
    pub(crate) fn unchecked_proposal_v2_from_test_vector() -> UncheckedProposal {
        ...
        UncheckedProposal {
            proposal: Proposal { psbt: PARSED_ORIGINAL_PSBT.clone(), params },
    ...
    fn test_extract_err_req_expiry() -> Result<(), BoxError> {
        ...
        let receiver = Receiver {
            state: UncheckedProposal {
                proposal: crate::receive::v1::test::proposal_from_test_vector(),
