...
fn mailbox_path_from_pubkey(pubkey: &HpkePublicKey) -> ShortId {
...
fn extract_err_req(
    ...
    let mailbox = mailbox(&session_context.directory, &session_context.id());
...
impl Receiver<Initialized> {
    ...
    > {
        let fallback_target = mailbox(&self.context.directory, &self.context.id());
...
impl Receiver<PayjoinProposal> {
    ...
    pub fn create_post_request(
        ...
        if let Some(e) = &self.context.e {
            ...
            let sender_mailbox = mailbox_path_from_pubkey(e);
            ...
            let receiver_mailbox = mailbox_path_from_pubkey(self.context.s.public_key());
            target_resource = self
                .context
                .directory
...
/// The mailbox for this Payjoin receiver session.
/// It consists of a directory URL and the session ShortID in the path.
fn mailbox(directory: &Url, id: &ShortId) -> Url {
...
pub(crate) fn pj_uri<'a>(
    ...
    let mut pj = mailbox(&session_context.directory, &id).clone();
