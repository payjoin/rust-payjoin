...
impl Receiver<Monitor> {
    ...
    ///
    /// This is the legacy, txid-only convenience: it can only probe the two transaction ids the
    /// receiver predicts, so it cannot detect a [`SettlementOutcome::Other`] (an unrelated spend
    /// of the contested outpoints leaves both lookups returning `None` forever), and it concludes
    /// on the first match — detection, not settlement. Prefer the
    /// [`classify`](Receiver::classify) / [`conclude`](Receiver::conclude) flow.
    ///
