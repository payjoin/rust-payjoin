...
//! sender and the receiver. It provides additional tools for persisting an existing Payjoin
//! session so that the receiver can have multiple on-going Payjoin sessions with different senders
//! without being continuously online for them.
//!
...
impl Receiver<UninitializedReceiver> {
    ...
    /// inactivity of either party, etc.).
    ///
...
/// receiver to substitute the original PSBT's outputs increase transaction privacy and batch UTXOs.
/// The receiver does not have to limit themselves to the address shared with the sender in the
/// original Payjoin URI, and can make substitutions on the existing outputs in the proposal.
///
...
impl Receiver<ProvisionalProposal> {
    ...
    ///   1. Subtract from the receiver outputs (and, if allowed by the sender, from their's) the fee costs to cover for the new inputs and/or outputs.
