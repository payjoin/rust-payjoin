...
use crate::receive::{v1, JsonReply, Proposal};
...
impl SessionHistory {
    ...
    fn get_unchecked_proposal(&self) -> Option<Proposal> {
        self.events.iter().find_map(|event| match event {
...
pub enum SessionEvent {
    ...
    UncheckedProposal((Proposal, Option<crate::HpkePublicKey>)),
    MaybeInputsOwned(),
    MaybeInputsSeen(),
    OutputsUnknown(),
...
mod tests {
    ...
    fn test_replaying_unchecked_proposal() -> Result<(), BoxError> {
        ...
        let test = SessionHistoryTest {
            ...
            expected_receiver_state: ReceiveSession::UncheckedProposal(Receiver {
                state: UncheckedProposal { proposal, session_context },
            }),
        };
        run_session_history_test(test)
    }
    ...
    fn test_replaying_unchecked_proposal_expiry() {
        ...
        let test = SessionHistoryTest {
            ...
            expected_receiver_state: ReceiveSession::UncheckedProposal(Receiver {
                state: UncheckedProposal { proposal, session_context: context },
            }),
        };
    ...
    fn getting_fallback_tx() -> Result<(), BoxError> {
        ...
        let test = SessionHistoryTest {
            ...
            expected_receiver_state: ReceiveSession::MaybeInputsOwned(Receiver {
                state: MaybeInputsOwned { proposal, session_context },
            }),
        };
        run_session_history_test(test)
    }
