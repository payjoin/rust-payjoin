...
impl SessionHistory {
    ...
    pub fn fallback_tx(&self) -> Option<bitcoin::Transaction> {
        self.events.iter().find_map(|event| match event {
            SessionEvent::CheckedBroadcastable() => Some(
    ...
    pub fn psbt_ready_for_signing(&self) -> Option<bitcoin::Psbt> {
        self.events.iter().find_map(|event| match event {
            SessionEvent::CommittedFees(psbt_context) => Some(psbt_context.payjoin_psbt.clone()),
            _ => None,
        })
    }
...
mod tests {
    ...
    fn test_session_event_serialization_roundtrip() {
        ...
        let test_cases = vec![
            ...
            SessionEvent::CheckedBroadcastable(),
            SessionEvent::CheckedInputsNotOwned(),
            SessionEvent::CheckedInputsNotSeenBefore(),
            ...
            SessionEvent::CommittedFees(provisional_proposal.state.psbt_context.clone()),
    ...
    fn getting_fallback_tx() -> Result<(), BoxError> {
        ...
        events.push(SessionEvent::CheckedBroadcastable());
    ...
    fn test_contributed_inputs() -> Result<(), BoxError> {
        ...
        events.push(SessionEvent::CheckedInputsNotSeenBefore());
        ...
        events.push(SessionEvent::CommittedFees(provisional_proposal.state.psbt_context.clone()));
    ...
    fn test_skipped_session_extract_err_request() -> Result<(), BoxError> {
        ...
        let session_history = SessionHistory { events: vec![SessionEvent::CheckedBroadcastable()] };
