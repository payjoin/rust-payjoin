...
pub(crate) enum InternalBuildSenderError {
    ...
    OriginalTxinSignatureNonAllSighashType {
        index: usize,
        sighash_type: bitcoin::psbt::PsbtSighashType,
    },
    OriginalTxinMalformedSignature {
        index: usize,
        error: crate::psbt::FinalSignatureError,
    },
    NoInputs,
    PayeeValueNotEqual,
    NoOutputs,
...
impl fmt::Display for BuildSenderError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match &self.0 {
            ...
            OriginalTxinSignatureNonAllSighashType { index, sighash_type } => write!(f, "input {index} of the original transaction is signed with sighash type {sighash_type}, which does not commit to all inputs and outputs"),
            OriginalTxinMalformedSignature { index, error } => write!(f, "input {index} of the original transaction has a malformed signature: {error}"),
...
impl std::error::Error for BuildSenderError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        ...
        match &self.0 {
            ...
            OriginalTxinSignatureNonAllSighashType { .. } => None,
            OriginalTxinMalformedSignature { error, .. } => Some(error),
            NoInputs => None,
