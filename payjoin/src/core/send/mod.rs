...
fn commits_to_all_inputs_and_outputs(sighash_type: PsbtSighashType) -> bool {
    match sighash_type.taproot_hash_ty() {
        Ok(TapSighashType::Default | TapSighashType::All) => true,
        // taproot_hash_ty rejects the ECDSA-only encodings, so fall back to
        // interpreting the type as ECDSA before giving up.
        _ => matches!(sighash_type.ecdsa_hash_ty(), Ok(EcdsaSighashType::All)),
    }
}
