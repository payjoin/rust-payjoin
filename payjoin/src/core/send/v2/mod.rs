...
use crate::persist::{
    InitialTransition, MaybeFatalTransition, MaybeSuccessTransitionWithNoResults,
};
...
impl SenderBuilder {
    ...
    ) -> Result<InitialTransition<SessionEvent, Sender<WithReplyKey>>, BuildSenderError> {
        let psbt_ctx =
    ...
    ) -> InitialTransition<SessionEvent, Sender<WithReplyKey>> {
        let with_reply_key = WithReplyKey::new(pj_param, psbt_ctx);
        InitialTransition::initialize(
...
impl Sender<WithReplyKey> {
    ...
    pub fn create_v2_post_request(
        ...
        let mut cleared_psbt = self.psbt_ctx.original_psbt.clone();
        clear_unneeded_fields(&mut cleared_psbt);
...
mod test {
    ...
    fn test_v2_sender_builder() {
        ...
        let req_ctx = SenderBuilder::new(PARSED_ORIGINAL_PSBT.clone(), pj_uri.clone())
            ...
            .expect("build on test vectorshould succeed")
            .save(&NoopSessionPersister::default())
            .expect("sender should succeed");
        assert_eq!(req_ctx.state.psbt_ctx.output_substitution, OutputSubstitution::Enabled);
        let req_ctx = SenderBuilder::new(PARSED_ORIGINAL_PSBT.clone(), pj_uri.clone())
