...
impl PjParam {
    /// Create a new v1 PjParam from a URL
    pub fn new(url: Url) -> Result<Self, PjParseError> {
        if url.scheme() == "https"
    ...
    pub fn endpoint(&self) -> Url { self.0.clone() }
}

impl std::fmt::Display for PjParam {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // Use the same display logic as the parent PjParam
        write!(f, "{}", self.0)
    }
}
