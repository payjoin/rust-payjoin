...
mod tests {
    ...
    fn test_exp_get_set() {
        ...
        assert_eq!(expiration(&url).expect("Expiry has been set but is missing on get"), exp_time);
    }
    ...
    fn test_errors_when_parsing_exp() {
        ...
        assert!(matches!(
            ...
            Err(ParseExpParamError::DecodeBech32(_))
        ));
        ...
        assert!(matches!(expiration(&invalid_hrp_exp_url), Err(ParseExpParamError::InvalidHrp(_))));
