...
mod tests {
    ...
    fn wrap_p2sh_script(script: &Script) -> Script {
        bitcoin::blockdata::script::Builder::new().push_slice(script.as_ref()).into_script()
    }
    ...
    fn test_p2sh() {
        ...
        let input_type = InputType::from_spent_input(
            &TxOut { script_pubkey: Script::new_p2sh(&script.script_hash()), value: 42 },
    ...
    fn test_p2wsh() {
        let script = Script::new_op_return(&[42]);
        let input_type = InputType::from_spent_input(
            &TxOut { script_pubkey: Script::new_v0_p2wsh(&script.wscript_hash()), value: 42 },
            &PsbtInput { final_script_sig: Some(script), ..Default::default() },
        )
        .unwrap();
    ...
    fn test_p2sh_p2wpkh() {
        ...
        let input_type = InputType::from_spent_input(
            &TxOut { script_pubkey: Script::new_p2sh(&segwit_script_hash), value: 42 },
