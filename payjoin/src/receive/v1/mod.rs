...
fn interleave_shuffle<'a, T: Clone + 'a, R: rand::Rng, I: IntoIterator<Item = &'a T>>(
    original: &mut Vec<T>,
    replacement_outputs: I,
    rng: &mut R,
) {
    let mut new_vec: Vec<T> = replacement_outputs.into_iter().cloned().collect();
    new_vec.shuffle(rng);

    let mut combined = Vec::with_capacity(original.len() + new_vec.len());
    // Initialize indices
    let mut original_index = 0;
    let mut new_index = 0;
    // Interleave elements
    while original_index < original.len() || new_index < new_vec.len() {
        if original_index < original.len() && (new_index >= new_vec.len() || rng.gen_bool(0.5)) {
            combined.push(original[original_index].clone());
            original_index += 1;
        } else {
            combined.push(new_vec[new_index].clone());
            new_index += 1;
        }
    }
    *original = combined;
