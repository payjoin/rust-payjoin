...
/// for each visit) payment receiver. For the latter, you should call [`Self::extract_tx_to_schedule_broadcast`] and
/// schedule the broadcast of the sender proposal, followed by [`Self::check_broadcast_suitability`] to check
...
///
/// Call [`Self::check_inputs_not_owned`] to proceed with the next typestate.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
...
///
/// Call [`Self::check_no_inputs_seen_before`] to proceed with the next typestate.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
...
/// money.
///
/// Call [`Self::identify_receiver_outputs`] to validate that you own the outputs and proceed with the next typestate.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
...
/// receiver to modify the original PSBT's outputs increase transaction privacy and batch UTXOs.
/// The receiver does not have to limit themselves to the address shared with the sender in the
/// original Payjoin URI, and can make modifications on the existing outputs in the proposal.
///
/// Call [`Self::commit_outputs`] to finalize the outputs and proceed.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
