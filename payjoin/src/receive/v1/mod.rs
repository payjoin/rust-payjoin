...
pub(crate) mod test {
    ...
    fn unchecked_proposal_below_min_fee() {
        ...
        match proposal.clone().check_broadcast_suitability(Some(min_fee_rate), |_| Ok(true)) {
            Ok(_) => panic!(
                "Broadcast suitability check should fail due to being below the min fee rate"
            ),
    ...
    fn empty_candidates_inputs() {
        ...
        match result {
            ...
            Err(err) => {
                assert_eq!(err, SelectionError::from(InternalSelectionError::Empty));
            }
        }
    }
    ...
    fn sender_specifies_excessive_fee_rate() {
        ...
        match psbt {
            Ok(_) => panic!("Payjoin exceeds receiver fee preference and should error"),
            Err(error) => match error {
    ...
    fn test_pjos_disabled() {
        ...
        match decreased_amount {
            Ok(_) => panic!("Payjoin receiver amount has been decreased and should error"),
            Err(error) => {
                assert_eq!(
                    error,
        ...
        match replace_receiver_script_pubkey {
            Ok(_) => panic!("Payjoin receiver script pubkey has been modified and should error"),
            Err(error) => {
                assert_eq!(
                    error,
                    OutputSubstitutionError::from(
    ...
    fn test_avoid_uih_one_output() {
        ...
        match avoid_uih {
            Ok(_) => panic!("Payjoin below minimum allowed outputs for avoid uih and should error"),
            Err(error) => {
                assert_eq!(
                    error,
