...
mod test {
    ...
    // ***************** Tests for Sender's payjoin proposal checklist from BIP-78 *****************
    #[test]
    ...
    fn test_key_path_found_in_proposal() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since it contains a key path"
        );
    ...
    fn test_partial_sig_found_in_proposal() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since it contains a partial signature"
        );
    ...
    fn test_sender_input_sequence_number_changed() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since the sequence numbers don't match"
        );
    ...
    fn test_sender_input_final_script_sig_is_present() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since an input has a final script sig"
        );
    ...
    fn test_sender_input_final_script_witness_is_present() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since an input has a final script witness"
        );
    ...
    fn test_receiver_input_is_not_finalized() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since the receiver's input hasn't been finalized"
        );
    ...
    fn test_receiver_input_missing_witness_info() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since the receiver's input doesn't have witness info"
        );
    ...
    fn test_process_proposal_when_missing_original_inputs() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since the proposal is missing inputs"
        );
    ...
    fn test_process_proposal_when_output_contains_key_path() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since the proposal has a key path in the output"
        );
    ...
    fn test_process_proposal_when_payee_output_has_disallowed_output_substitution(
        ...
        ctx.original_psbt.unsigned_tx.output.get_mut(0).unwrap().script_pubkey = ctx.payee.clone();
        ctx.output_substitution = OutputSubstitution::Disabled;

        let result = ctx.process_proposal(proposal);
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since the psbt context specified that output substitution is disabled"
        );
    ...
    fn test_process_proposal_when_output_value_decreased() -> Result<(), BoxError> {
        ...
        proposal.unsigned_tx.output.get_mut(0).unwrap().value =
            ctx.original_psbt.unsigned_tx.output.get_mut(0).unwrap().value - Amount::from_sat(1);

        let result = ctx.process_proposal(proposal);
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since the output value decreased"
        );
    ...
    fn test_process_proposal_when_output_missing() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since the outputs are missing"
        );
    ...
    fn test_absolute_fee_less_than_original_psbt() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since the fee is less than the original psbt"
        );
    ...
    fn test_payee_took_contributed_fee() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since the payee took the contributed fee"
        );
    ...
    fn test_fee_contribution_pays_output_size_increase() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since the fee contribution pays output size increases"
        );
    ...
    fn test_fee_rate_below_minimum() -> Result<(), BoxError> {
        ...
        assert!(
            result.is_err(),
            "Process proposal should reject proposal since the fee rate is below the minimum"
        );
