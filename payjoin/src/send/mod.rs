...
pub(crate) mod test {
    ...
    fn test_determine_fees() -> Result<(), BoxError> {
        ...
        assert_eq!((*fee_contribution.as_ref().expect("Failed to retreive fees")).unwrap().vout, 1);
        assert_eq!(
    ...
    fn test_self_pay_change_index() -> Result<(), BoxError> {
        ...
        //check_change_index will panic when change output is equal to psbt script
    ...
    fn test_find_change_index() -> Result<(), BoxError> {
        ...
        assert_eq!((*fee_contribution.as_ref().expect("failed to retreive fees")).unwrap().vout, 0);
        assert_eq!(
            (*fee_contribution.as_ref().expect("failed to retreive fees")).unwrap().max_amount,
            Amount::from_sat(1000)
        );
    ...
    fn test_single_payee_amount_mismatch() -> Result<(), BoxError> {
        ...
        //check_single_payee will panic when psbt amount is different from uri amount
    ...
    fn test_clear_unneeded_fields() -> Result<(), BoxError> {
        ...
        //We will add 2 additional keys to insert that should be stripped
        let _ = proposal.inputs[0].tap_internal_key.insert(XOnlyPublicKey::from_str(
            "4f65949efe60e5be80cf171c06144641e832815de4f6ab3fe0257351aeb22a84",
        )?);
        let _ = proposal.outputs[0].tap_internal_key.insert(XOnlyPublicKey::from_str(
            "4f65949efe60e5be80cf171c06144641e832815de4f6ab3fe0257351aeb22a84",
        )?);
    ...
    #[test]
    fn official_vectors() -> Result<(), BoxError> {
        let original_psbt = Psbt::from_str(ORIGINAL_PSBT)?;
        eprintln!("original: {:#?}", original_psbt);
    ...
    fn test_disable_output_substitution_query_param() -> Result<(), BoxError> {
        let url = serialize_url(Url::parse("http://localhost")?, true, None, FeeRate::ZERO, "2")
            .map_err(|e| format!("Failed to serialize url: {e}"))?;
