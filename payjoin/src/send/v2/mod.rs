...
mod test {
    ...
    fn create_sender_context() -> Result<super::Sender<super::SenderWithReplyKey>, BoxError> {
        let endpoint = Url::parse("http://localhost:1234")?;
        let mut sender = super::Sender {
            state: SenderWithReplyKey {
                v1: v1::Sender {
        ...
        sender.state.v1.endpoint.set_exp(SystemTime::now() + Duration::from_secs(60));
        sender.state.v1.endpoint.set_receiver_pubkey(HpkeKeyPair::gen_keypair().1);
        sender.state.v1.endpoint.set_ohttp(OhttpKeys(
    ...
    // TODO: need to rewrite this test to serialize and deserialize the sender session events
    // #[test]
    ...
    fn test_serialize_v2() -> Result<(), BoxError> {
        let sender = create_sender_context()?;
        let body = serialize_v2_body(
            &sender.state.v1.psbt,
            sender.state.v1.output_substitution,
            sender.state.v1.fee_contribution,
            sender.state.v1.min_fee_rate,
        );
    ...
    fn test_extract_v2_success() -> Result<(), BoxError> {
        ...
        assert_eq!(
            ...
            format!("{}{}", EXAMPLE_URL.clone(), sender.state.v1.endpoint.join("/")?)
        );
        assert_eq!(context.endpoint, sender.state.v1.endpoint);
        assert_eq!(context.psbt_ctx.original_psbt, sender.state.v1.psbt);
        Ok(())
    }
    ...
    fn test_extract_v2_fails_missing_pubkey() -> Result<(), BoxError> {
        ...
        sender.state.v1.endpoint.set_fragment(Some(""));
        sender.state.v1.endpoint.set_exp(SystemTime::now() + Duration::from_secs(60));
        sender.state.v1.endpoint.set_ohttp(OhttpKeys(
            ohttp::KeyConfig::new(KEY_ID, KEM, Vec::from(SYMMETRIC)).expect("valid key config"),
        ));
    ...
    fn test_extract_v2_fails_when_expired() -> Result<(), BoxError> {
        ...
        sender.state.v1.endpoint.set_exp(exp_time);
        let ohttp_relay = EXAMPLE_URL.clone();
        let result = sender.extract_v2(ohttp_relay);
