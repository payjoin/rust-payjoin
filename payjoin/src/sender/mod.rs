    ...
    fn check_fees(
        ...
        if self.min_fee_rate > FeeRate::ZERO {
            ...
            let total_weight = if in_stats.inputs_with_witnesses == 0 {
                ...
                proposed_psbt_fee / total_weight >= self.min_fee_rate,
                FeeRateBelowMinimum
            );
        }
        Ok(())
            ...
            match original_inputs.peek() {
                    ...
                    if proposed.txin.previous_output == original.txin.previous_output =>
                {
                    check_eq!(
                        ...
                        proposed.psbtin.witness_utxo.is_none(),
                    ...
                    let prevout = original
                        .previous_txout()
                        .expect("We've validated this before");
...
fn find_change_index(
    ...
    Ok(Some((
        check_fee_output_amount(output, fee, clamp_fee_contribution)?,
        index,
    )))
}
...
fn determine_fee_contribution(
    ...
    Ok(match params.fee_contribution {
        ...
        Some((fee, Some(index))) => Some(check_change_index(
            psbt,
            payee,
            fee,
            index,
            params.clamp_fee_contribution,
        )?),
        None => None,
    })
}
...
fn serialize_url(
    ...
    if disable_output_substitution {
        url.query_pairs_mut()
            .append_pair("disableoutputsubstitution", "1");
    }
...
pub(crate) fn from_psbt_and_uri(
    ...
    let zeroth_input = psbt
        .input_pairs()
        .next()
        .ok_or(InternalCreateRequestError::NoInputs)?;
