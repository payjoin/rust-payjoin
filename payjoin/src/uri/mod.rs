...
mod tests {
    ...
    fn test_serialize_pjos() {
        let uri = "bitcoin:12c6DSiU4Rq3P4ZxziKxzrL5LmMBrzjrJX?pj=HTTPS://EXAMPLE.COM/\
                   %23OH1QYPM5JXYNS754Y4R45QWE336QFX6ZR8DQGVQCULVZTV20TFVEYDMFQC";
        let expected_is_disabled = "pjos=1";
        let expected_not_disabled = "pjos=0";
        let mut pjuri = Uri::try_from(uri).unwrap().assume_checked().check_pj_supported().unwrap();

        pjuri.extras.disable_output_substitution = true;
        assert!(
        ...
        assert!(
            pjuri.to_string().contains(expected_not_disabled),
            "Pj uri should contain param: {}, but it did not",
            expected_not_disabled
        );
    }
}
