...
impl std::str::FromStr for OhttpKeys {
    type Err = OhttpKeysParseError;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let bytes = base64::decode_config(s, base64::URL_SAFE_NO_PAD)
            .map_err(OhttpKeysParseError::Base64DecodeError)?;
        OhttpKeys::decode(&bytes).map_err(OhttpKeysParseError::KeyConfigDecodeError)
    }
}
...
#[derive(Debug)]
pub enum OhttpKeysParseError {
    Base64DecodeError(base64::DecodeError),
    KeyConfigDecodeError(ohttp::Error),
}

impl std::fmt::Display for OhttpKeysParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            OhttpKeysParseError::Base64DecodeError(e) =>
                write!(f, "Failed to decode base64: {}", e),
            OhttpKeysParseError::KeyConfigDecodeError(e) =>
                write!(f, "Failed to decode KeyConfig: {}", e),
        }
    }
}

impl std::error::Error for OhttpKeysParseError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            OhttpKeysParseError::Base64DecodeError(e) => Some(e),
            OhttpKeysParseError::KeyConfigDecodeError(e) => Some(e),
        }
    }
}
