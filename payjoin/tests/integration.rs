        ...
        async fn v2_to_v2_p2pkh_async() -> Result<(), BoxSendSyncError> {
            ...
            monitoring_payment
                .check_payment_async(|_| async {
                    // NOTE: this was changed from panic! to None bc the async
                    // transaction_exists will precompute the results for payjoin
                    // proposal and fallback, and when fallback tx is not found the
                    // panic will cause this test to fail.
                    Ok(None)
                })
                .await
                .save(&recv_persister)
