...
class TestReceiveMod(unittest.TestCase):         
    def get_proposal_from_test_vector(self)-> pdk.UncheckedProposal:
        try:
            ...
            unchecked_proposal = pdk.UncheckedProposal.from_request(body,"?maxadditionalfeecontribution=182?additionalfeeoutputindex=0", headers )
    ...
    def test_unchecked_proposal_unlocks_after_checks(self):
        try:
            ...
            # In Python, you would replace this with the appropriate function call or object instantiation
            unchecked_proposal = pdk.UncheckedProposal.from_request(body,"?maxadditionalfeecontribution=182?additionalfeeoutputindex=0", headers )
            proposal = unchecked_proposal.assume_interactive_receiver().check_inputs_not_owned(ScriptOwnershipChecker()).check_no_mixed_input_scripts().check_no_inputs_seen_before(OutputAvailabilityChecker).identify_receiver_outputs(ScriptOwnershipChecker)
