...
use crate::{AddressType, CachedOutputs, Input};
...
impl BitcoindClient {
	pub fn new(config: BitcoindConfig) -> Result<Self, bitcoincore_rpc::Error> {
		let bitcoind = (match &config.cookie {
			...
			None => bitcoincore_rpc::Client::new(
				&config.rpc_host,
				bitcoincore_rpc::Auth::UserPass(config.rpc_user.clone(), config.rpc_pass.clone()),
			),
		})
	...
	pub fn create_wallet(
		&self, wallet_name: String, disable_private_keys: Option<bool>, blank: Option<bool>,
		passphrase: Option<String>, avoid_reuse: Option<bool>,
	) -> Result<String, bitcoincore_rpc::Error> {
		let client = self.get_rpc_client();
		match client.create_wallet(
			&wallet_name.as_str(),
	...
	pub fn create_psbt(
		&self, inputs: Vec<Input>, outputs: HashMap<String, u64>, locktime: Option<i64>,
		...
		return match client.create_psbt(
			pared_inputs.as_slice(),
			&parsed_outputs,
			locktime,
			replaceable,
		) {
	...
	pub fn get_new_address(
		&self, label: Option<&str>, address_type: Option<AddressType>,
	) -> Result<String, bitcoincore_rpc::Error> {
		let client = self.get_rpc_client();
		match client.get_new_address(label, address_type.map(|x| x.into())) {
			Ok(e) => Ok(e.assume_checked().to_string()),
			Err(e) => panic!("{:?}", e),
		}
	}
	pub fn is_address_mine(
		self, script: &bitcoin::Script, network: bitcoin::Network,
