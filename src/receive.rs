...
impl UncheckedProposal {
	pub fn from_request(
		...
		match PdkUncheckedProposal::from_request(body, query.as_str(), headers) {
			Ok(e) => Ok(UncheckedProposal { internal: e }),
			Err(e) => anyhow::bail!("{e}"),
		}
	}
	...
	pub fn check_can_broadcast(
	...
	) -> Result<MaybeInputsOwned, anyhow::Error> {
		...
		match res {
			Ok(e) => Ok(MaybeInputsOwned { internal: e }),
			Err(e) => anyhow::bail!("{e}"),
		}
	}
...
impl MaybeInputsSeen {
	...
	pub fn check_no_inputs_seen_before(
		self, is_known: impl IsOutoutKnown,
	) -> Result<OutputsUnknown, anyhow::Error> {
		match self.internal.check_no_inputs_seen_before(|outpoint| {
...
mod test {
	...
	fn get_proposal_from_test_vector() -> Result<UncheckedProposal, anyhow::Error> {
