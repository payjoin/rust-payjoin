...
pub(crate) struct Configuration {
	pub(crate) internal: RwLock<PdkConfiguration>,
}
...
impl Configuration {
	...
	pub fn non_incentivizing() -> Arc<Self> {
		Arc::new(Self { internal: RwLock::new(PdkConfiguration::non_incentivizing()) })
	}
	...
	pub fn always_disable_output_substitution(self, disable: bool) -> Self {
		{
			...
			let taken_config = std::mem::replace(
				&mut *data_guard,
				PdkConfiguration::with_fee_contribution(bitcoin::Amount::ZERO, None),
			);
			*data_guard = taken_config.always_disable_output_substitution(disable);
		}
		self
	}
	...
	pub fn clamp_fee_contribution(mut self, clamp: bool) -> Self {
		{
			...
			*data_guard = taken_config.clamp_fee_contribution(clamp);
		}
		self
	}
	...
	pub fn min_fee_rate_sat_per_vb(self, fee_rate: u64) -> Self {
		{
			...
			*data_guard = taken_config.min_fee_rate_sat_per_vb(fee_rate);
		}
		self
	}
}
