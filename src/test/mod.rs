...
fn integration_test() {
    ...
    outputs.insert(pj_uri.clone().address(), pj_uri.clone().amount().unwrap());
...
fn handle_pj_request(
    ...
    payjoin.substitute_output_address(
        crate::Address
            ::from_str(receiver_substitute_address.to_string().as_str())
...
impl IsOutoutKnown for MockOutputOwned {
    fn is_known(&self, _: crate::OutPoint) -> Result<bool, payjoin::Error> {
        Ok(false)
    }
}
impl IsScriptOwned for MockScriptOwned {
    fn is_owned(&self, script: ScriptBuf) -> Result<bool, payjoin::Error> {
        {
