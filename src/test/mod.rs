...
use crate::uri::Uri;
use crate::{ Error, Network, ScriptBuf };

#[test]
fn integration_test() {
    let _ = env_logger::try_init();
    let bitcoind_exe = std::env
        ::var("BITCOIND_EXE")
        .ok()
    ...
    let checked_payjoin_proposal_psbt = match
        PartiallySignedTransaction::process_response(pj_req.context, response)
    {
        Ok(e) => e,
...
fn handle_pj_request(
    req: Request,
    headers: Headers,
    receiver: Arc<bitcoincore_rpc::Client>
) -> String {
    ...
    let proposal = proposal
        .check_can_broadcast(Arc::new(TestBroadcast(receiver.clone())))
    ...
    let outpoint_to_contribute = crate::OutPoint {
        txid: selected_utxo.txid.to_string(),
    ...
    payjoin.substitute_output_address(
        crate::Address
            ::new(receiver_substitute_address.to_string().as_str().to_owned())
    ...
    fn is_owned(&self, script: ScriptBuf) -> Result<bool, anyhow::Error> {
        {
            ...
            let address = crate::Address::from_script(Arc::new(script), network).unwrap();
